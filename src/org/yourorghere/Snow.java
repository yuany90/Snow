...
public class Snow {
   ...
   public float m_fXSpeed = 0.0f;
   // Variable for user to control the particles' speed in the Y direction.
   public float m_fYSpeed = 0.0f;
   public boolean resetVertexs = false;
   /**
   ...
   public Snow() {
      ...
      m_oCanvas = new GLCanvas();
      // Create the OpenGL window used to display OpenGL objects.
      createGLWindow("JOGL Port for NeHe OpenGL", WIN_WIDTH, WIN_HEIGHT, m_nBits);
   ...
   private void createGLEnabledFrame(int width, int height, int bit) {
      ...
      m_oCanvas.addGLEventListener(render);
    ...
    */
   public void showError(Exception error, String errorMessage) {
    ...
    */
   public void changeYGravity(boolean increase) {
      if (increase) {
    ...
    *        pull increases). 
    */
   public void changeXGravity(boolean increase) {
      if (increase) {
    ...
    */
   public void changeSlowDown(boolean increase) {
      if (increase) {
         m_fSlowDown += 0.01f;
      } else {
         m_fSlowDown += -0.01f;
    ...
    *        
    */
   public void changeXSpeed(boolean increase) {
      if (increase) {
         m_fXSpeed += 0.5f;
      } else {
         m_fXSpeed += -0.5f;
    ...
    *        
    */
   public void changeYSpeed(boolean increase) {
      if (increase) {
         m_fYSpeed += 0.5f;
      } else {
         m_fYSpeed += -0.5f;
